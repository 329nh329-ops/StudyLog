...
> このドキュメントは実装・レビューを経て、実際のBackend/Frontend実装と整合するよう更新された正式な仕様書です。当初の仕様からの主な変更点は [46. 初期仕様からの変更点](#46-初期仕様からの変更点) にまとめています。プロジェクトの目的や背景は [docs/要件定義.md](../要件定義.md) を、セットアップ手順や技術スタックは [README.md](../../README.md) を参照してください。
...
| API形式 | REST API |
| データ形式 | JSON |
| グラフ描画 | Recharts（ダッシュボードの日別/カテゴリ別/月別学習時間グラフに使用） |
...
当初はEC2上で`docker compose up --build`を実行し、git clone直後にビルド・起動まで完結させる方式を想定していた。しかし実機検証の結果、EC2インスタンス（t3.micro、メモリ1GB）上でNext.jsの本番ビルドを実行するとメモリ不足でOSごと応答不能になることが判明した。
...
- 1〜25文字
- ユニーク
- **文字種：半角英数字・ハイフン（`-`）・アンダースコア（`_`）のみ**（`^[A-Za-z0-9_-]{1,25}$`）
...
「今日」の判定は**JST（Asia/Tokyo）基準**で行う。サーバーはUTCで動作しているため、単純な現在日時取得ではなくJSTに変換した上で判定している。
...
ヘッダー・サイドバーの配色はDesign Tokenで管理している。Design Tokenおよび共通UIコンポーネントの詳細は [45. Design System / 共通UIコンポーネント](#45-design-system--共通uiコンポーネント) を参照。
...
| GET | `/api/admin/users/{id}` | 単一ユーザー情報 |
...
すべての変更はPull Requestを経由してmainブランチに取り込み、CI（自動テスト・Lint）が成功しない限りマージできない運用としている。仕様上の問題や矛盾を発見した場合は独断で仕様を変更せず、実装前に確認・報告するプロセスを徹底している。
...
## 45. Design System / 共通UIコンポーネント

GitHub / Vercel / Linearのようなダークモード中心・エンジニア向けのUIを目指し、Modern Dark Design Systemを採用している。ライト/ダークの切り替え機能は設けず、ダークテーマを標準（固定）テーマとする。
...
### 45.2 共通UIコンポーネント（`frontend/components/ui/`）
...
開発の過程で、実装・レビューを通じて当初の仕様から以下の点を確認・追記した。

| 項目 | 変更内容 |
| --- | --- |
| ユーザー名の文字種 | 半角英数字・ハイフン・アンダースコアのみに制限することを明確化（`^[A-Za-z0-9_-]{1,25}$`） |
| 学習時間の上限 | 上限1440分（24時間）であることを明確化 |
| 「今日」の判定基準 | サーバーの実行環境がUTCで動作するため、JST（Asia/Tokyo）基準に変換した上で判定する方式に修正 |
| CSRF対策 | Double Submit Cookie方式の具体的な実装内容（`csrf_token` Cookie発行、`X-CSRF-Token`ヘッダー検証、認証チェックとの評価順序）を追記 |
| カテゴリ名のUNIQUE制約 | 論理削除後の同名再登録を可能にするため、生成カラムによる部分ユニーク制約を採用 |
| 単一ユーザー取得API | `GET /api/admin/users/{id}` を追加 |
| APIエラーコード | `CSRF_TOKEN_INVALID`（403）、`USER_NOT_FOUND`（404）を追加 |
| UIデザイン | ダークモード中心のデザインシステム（Design Token・共通UIコンポーネント）を導入 |
